...
const images = ['nav-fix-desktop', 'nav-fix-mobile', 'nav-fix-mobile-open'];

const cards = images.map(name => {
  const buf = readFileSync(`snapshots/${name}.png`);
  const b64 = buf.toString('base64');
  const label = name.replace('nav-fix-', '').toUpperCase().replace('-', ' ');
...
writeFileSync('snapshots/gallery-nav.html', html);
